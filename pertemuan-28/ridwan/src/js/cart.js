...
window.hendelRemoveFromCart = function (itemId) {
  console.log(itemId);

   // konfirmasi sebelum mengahpus
    Swal.fire({
      title: "Apa anda yakin?",
      text: "Produk akan dihapus dari kernjang!",
      icon: "warning",
      ...
      if (result.isConfirmed) {
        ...
        const response = await API.admin.removeFromCart(itemId);
        if(response.ok){
          showCartCount();
          showProducts();
          showCartTotalPrice();
          Swal.fire({
            ...
            text: "Produk sudah berhasil di hpus dari keranjang",
            icon: "success"
          });
        }
      }
