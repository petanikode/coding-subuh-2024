...
export const API = {
  ...
  updateCartItemQty: (item) =>
    updateRequest(`${BASE_URL}/cart/update-qty/${item.get("id")}`, item),
  checkout: (items) => sendPostRequest(`${BASE_URL}/checkout`, { items }),
  getTransactions: (_) => fetchWithToken(`${BASE_URL}/transactions`),
  ...
  admin: {
    ...
    updateProduct: (product) =>
      updateRequest(`${BASE_URL}/admin/products/${product.get("id")}`, product),
    getAllTransactions: (_) => fetchWithToken(`${BASE_URL}/admin/transactions`),
    getPaidTransactions: (_) =>
      fetchWithToken(`${BASE_URL}/admin/transactions?status=PAID`),
  },
};
